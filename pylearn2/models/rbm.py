...
class ConvRBM(Block, Model):
    """
    A base interface for convolutional RBMs, implementing the binary-binary case.
...
class GaussianBinaryConvRBM(RBM):
    """
    An Convolutional RBM with Gaussian visible units and binary hidden units.
    """
    def __init__(self, energy_function_class,
            nvis = None,
            nhid = None,
        ...
        init_bias_hid : scalar or 1-d array of length `nhid`
            Initial value for the biases on hidden units.
        """
        super(GaussianBinaryCRBM, self).__init__(nvis = nvis, nhid = nhid,
                                                transformer = transformer,
                                                vis_space = vis_space,
                                                hid_space = hid_space,
                                                irange = irange, rng = rng,
