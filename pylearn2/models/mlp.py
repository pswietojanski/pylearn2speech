...
class MLP(Layer):
    ...
    def set_params(self, params_dict):
        for layer in self.layers:
            for param in layer.get_params():
                if not (param.name in params_dict):
    ...
    def annealed_dropout_fprop(self, state_below, default_input_include_prob=0.5,
                      input_include_probs=None, default_input_scale=2.,
                      input_scales=None, per_example=True):
        """
        state_below: The input to the MLP
...
#            print 'Input space of %s has dim %i with shape:'%(self.layer_name, self.input_dim), self.input_space.shape
#        elif isinstance(self.input_space, VectorSpace): 
#            print 'Input space of %s has dim:'%self.layer_name, self.input_space.dim
#        else:
#            pass
#        
#        if isinstance(self.output_space, Conv2DSpace):        
#            print 'Output space of %s has dim %i with shape:'%(self.layer_name, self.input_dim), self.output_space.shape
#        elif isinstance(self.output_space, VectorSpace): 
#            print 'Output space of %s has dim:'%self.layer_name, self.output_space.dim
#        else:
#            pass       
...
class Tanh(Linear):
    ...
    """
    def __init__(self, amp=1.7159, slope=0.66667, **kwargs):
    ...
    def fprop(self, state_below):
        p = self._linear_part(state_below)
        p = self.amp*T.tanh(self.slope*p)
        return p
...
class LpLayer(Linear):

    def __init__(self, lp_order = 2.0, **kwargs):
        super(LpLayer, self).__init__(**kwargs)
        self.lp_order = lp_order

    def fprop(self, state_below):
        p = self._linear_part(state_below)
        p = T.sqrt((1.0/self.input_dim)*T.abs_(p))
        return p
...
class Conv1DSigmoidStrided(Layer):
    """
        WRITEME
    """
    ...
    def __init__(self,
                 ...
                 cross_channel_pooling="none", #pre post or none
                 reshape_output_space_bandwise=False):
        ...
        if cross_channel_pooling in ["pre", "post"] and channelwise_conv is False:
            raise AssertionError("Can't do maxpool channels when chanelwise_conv is false")
    ...
    def set_input_space(self, space):
        ...
        if self.border_mode == 'valid':
            ...
            raise NotImplementedError('Full convolution for 1D not implemented')
        
        output_channels = self.output_channels
        if self.channelwise_conv:
            output_channels = self.output_channels*self.input_space.num_channels
        ...
        if self.irange is not None:
            assert self.sparse_init is None
            self.transformer = conv1d.make_random_conv1D(
                    irange = self.irange,
                    input_space = self.input_space,
                    output_space = self.detector_space,
                    kernel_shape = self.kernel_shape,
                    ...
                    border_mode = self.border_mode,
                    rng = rng,
                    tied_input_channels = self.tied_input_channels,
                    channelwise_conv = self.channelwise_conv)
        elif self.sparse_init is not None:
            self.transformer = conv1d.make_sparse_random_conv1D(
                    num_nonzero = self.sparse_init,
                    input_space = self.input_space,
                    output_space = self.detector_space,
                    kernel_shape = self.kernel_shape,
        ...
        #print 'W shape ', W.get_value().shape
        ...
        #print 'Input shape and channels: ', self.input_space.shape, self.input_space.num_channels
        #print 'Detector space and channels: ', self.detector_space.shape, self.detector_space.num_channels
        
        #log.info('Input shape: ', self.input_space.shape)
        #log.info('Detector space: ', self.detector_space.shape)
        
        assert self.pool_type in ['max', 'mean', 'none']
        ...
        if self.pool_type == 'max':
            ...
            dummy_p = dummy_detector
        dummy_p = dummy_p.eval()

        assert self.cross_channel_pooling in ['pre', 'post', 'none']

        if self.cross_channel_pooling in ["post", "pre"] and self.channelwise_conv: 
            #cross-channel pooling will be applied on the top so the output 
            # size will be self.output_channels rather than self.output_channels*self.input_space.num_channels
            output_channels = self.output_channels
    
        num_feats = dummy_p.shape[3]
        num_bands = dummy_p.shape[3]
        if  self.reshape_output_space_bandwise: 
            #reorder and flatten the conv bands so the upper cnn layer can apply set of (large) filters
            # i.e. we want to have a single input channel which is a concatenated and band-wise shuffled version of output filters
            num_feats = num_bands * output_channels
            permute = np.zeros((num_feats, num_feats), dtype=config.floatX)
            for k in xrange(output_channels):
                for l in xrange(0, num_bands):
                    i = output_channels*l+k
                    j = k*num_bands+l
                    permute[i,j]=1 #move on place ith elem jth when np.dot(p,permute)
            self.permute = sharedX(permute, 'band_permute')
            output_channels = 1 #output channels dimension will be collapsed and will expand 1 topoligical axis
    
        self.output_space = Conv2DSpace(shape=[dummy_p.shape[2], num_feats],
                num_channels = output_channels, axes = ('b', 'c', 0, 1) )

        #print 'Output space and channels ', self.output_space.shape, output_channels
    ...
    def fprop(self, state_below):
        ...
        if self.cross_channel_pooling in ["pre", "post"]: 
            #if true z has input_space.num_channels more featrue maps we need to apply the same bias across these groups
            b = T.concatenate([b for i in xrange(self.input_space.num_channels)], axis=1)
        ...
        if self.reshape_output_space_bandwise:
            #p is (batch_size, nb filters, 1, feats)
            batch_size = self.mlp.batch_size
            num_feats = self.output_space.num_channels*self.output_space.shape[1]
            #p = T.dot(p.reshape((batch_size,1,1,num_feats)), self.permute)
            p = p.reshape((batch_size,1,1,num_feats))
...
class Conv1DRectifiedLinearStrided(Layer):
    """
        WRITEME
    """
...
class MaxoutConv1DStrided(Layer):
    """
        WRITEME
    """
