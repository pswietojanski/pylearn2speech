...
class MonitorBasedLRNewBob(TrainExtension):
    """
    Exponential learning rate schema that is commonly used in speech applications.
    """
    ...
    def on_monitor(self, model, dataset, algorithm):
        ...
        try:
            ...
            raise KeyError('There is no monitoring channel named ' + \
                    self.channel_name + '. You probably need to specify '
                    'dataset_name in the MonitorBasedLRNewBob constructor.')
        ...
        if len(v) < 1:
            ...
            raise ValueError("""For some reason there are no monitor entries,
                    yet the MonitorBasedLRNewBob has been called. This should NEVER happen.
                    The Train object should call the monitor once on initialization, then
                    call the callbacks.
                    It seems you are either calling the callback manually rather than as part of
                    a training algorithm, or there is a problem with the Train object.""")
        ...
        if (self.ramping):
            rval *= self.scale_by
            log.info('learning rate shrinked to %.6f'%rval)
        elif ((v[-2] - v[-1]) < self.ramping_threshold):
            if self.do_ramping: self.ramping = True
            rval *= self.scale_by
